...
<div class="lg:container min-h-[calc(100vh-theme(space.16))]">
      ...
      <div class="flex-1">
        <a href="/dashboard" aria-current="page" aria-label="Dashboard" class="flex-0 btn btn-ghost px-2">
          <div class="font-title text-primary inline-flex text-lg transition-all duration-200 md:text-3xl">
            <span class="lowercase text-primary">members</span>
        ...
        <a class="btn btn-ghost btn-circle" href="/messages" aria-current="page" aria-label="Messages">
          <div class="indicator">
            <svg
              ...
              /></svg
            >
            {#if hasNotifications}<span class="badge badge-xs badge-primary indicator-item" />{/if}
          </div>
        </a>
